...
class AdapterHTTPError(IOError):
    ...
    :class:`geocodepy.exc.GeocoderServiceError`.
...
class BaseAdapter(abc.ABC):
    ...
    def __init__(self, *, proxies, ssl_context):
        ...
        :param dict proxies: An urllib-style proxies dict, e.g.
            ...
            See :attr:`geocodepy.geocoders.options.default_proxies` (note
            that Adapters always receive a dict: the string proxy
            is transformed to dict in the base
            :class:`geocodepy.geocoders.base.Geocoder` class.).
    ...
    def get_json(self, url, *, timeout, headers):
        ...
        :class:`geocodepy.exc.GeocoderParseError` must be raised if
    ...
    def get_text(self, url, *, timeout, headers):
        ...
        - :class:`geocodepy.adapters.AdapterHTTPError` should be raised if the response
          was successfully retrieved but the status code was non-successful.
        - :class:`geocodepy.exc.GeocoderTimedOut` should be raised when the request
          times out.
        - :class:`geocodepy.exc.GeocoderUnavailable` should be raised when the target
          host is unreachable.
        - :class:`geocodepy.exc.GeocoderServiceError` is the least specific error
...
class URLLibAdapter(BaseSyncAdapter):
    ...
    def _encode_multipart_form_data(self, buffer, boundary, name, value):
