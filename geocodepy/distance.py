...
``geocodepy.distance.distance`` currently uses :class:`.geodesic`.
...
of :code:`(lat, lon)` tuples::
    ...
    >>> from geocodepy import distance
...
calculate the length of a path::
    ...
    >>> from geocodepy import Nominatim
    >>> d = distance.distance
...
from geocodepy.units import radians
...
def lonlat(x, y, z=0):
    """
    ``geocodepy.distance.distance`` accepts coordinates in ``(y, x)``/``(lat, lon)``
    ...
    ``(x, y)``/``(lon, lat)`` format to a :class:`geocodepy.point.Point` instance.

    Example::

        >>> from geocodepy.distance import lonlat, distance
...
class Distance:
    ...
    Can be used for units conversion::
        ...
        >>> from geocodepy.distance import Distance
        >>> Distance(miles=10).km
        16.09344

    Distance instances have all *distance* properties from :mod:`geocodepy.units`,
    ...
    def destination(self, point, bearing, distance=None):
        ...
        Example: a point 10 miles east from ``(34, 148)``::
            ...
            >>> import geocodepy.distance
            >>> geocodepy.distance.distance(miles=10).destination((34, 148), bearing=90)
            Point(33.99987666492774, 148.17419994321995, 0.0)

        :param point: Starting point.
        :type point: :class:`geocodepy.point.Point`, list or tuple of ``(latitude,
        ...
        :param distance: Distance, can be used to override
            this instance::
                ...
                >>> from geocodepy.distance import distance, Distance
...
distance=Distance(100))
        ...
        :rtype: :class:`geocodepy.point.Point`
        """
        raise NotImplementedError("Distance is an abstract class")
    ...
    def set_ellipsoid(self, ellipsoid):
        if isinstance(ellipsoid, str):
            try:
                ...
                raise Exception(
                    "Invalid ellipsoid. See geocodepy.distance.ELLIPSOIDS"
                )
        else:
