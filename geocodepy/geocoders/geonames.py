...
class GeoNames(Geocoder):
    ...
    def __init__(
        ...
        :param str scheme:
            See :attr:`geocodepy.geocoders.options.default_scheme`. Note that
            at the time of writing GeoNames doesn't support `https`, so
            the default scheme is `http`. The value of
            :attr:`geocodepy.geocoders.options.default_scheme` is not respected.
    ...
    def reverse_timezone(self, query, *, timeout=DEFAULT_SENTINEL):
        ...
        timezone is used to produce the :class:`geocodepy.timezone.Timezone`.
        ...
        :rtype: :class:`geocodepy.timezone.Timezone`.
        """
        ensure_pytz_is_installed()
